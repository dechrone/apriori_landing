...
/** A stored A/B comparator result is an AbReport (verdict + annotated_screens),
 * not a single-flow SimulationData. Shared links must render it as the A/B
 * report — not coerce it into an empty funnel view. */
function isAbReport(result: unknown): result is AbReport {
  if (!result || typeof result !== "object") return false;
  const r = result as Partial<AbReport>;
  return !!(r.verdict && r.annotated_screens && Array.isArray(r.annotated_screens.screens));
}
...
export default function SharedSimulationPage() {
  ...
  // A/B comparator runs persist an AbReport — render the full A/B report
  // (verdict + lever analysis + screens) instead of degrading to a funnel.
  // The design-combiner fused image is persisted separately and isn't part of
  // the shared record, so it's omitted here (combinerStalled hides the live
  // "generating…" affordance on this static view).
  if (isAbReport(result)) {
    return (
      <>
        {banner}
        <div className={`${playfair.variable} ${dmSans.variable}`}>
          <AbReportView data={result} designCombiner={null} combinerStalled={true} readOnly />
        </div>
      </>
    );
  }

  const sim = result as SimulationData;
  const completionRate = sim.summary?.completion_rate_pct ?? 0;
  const totalPersonas = sim.summary?.total_personas ?? 0;

  const flowData: FlowAnalysisData = {
    meta: {
      product: record.name || sim.flow_name || "Product Flow",
      flow: sim.flow_name || record.name || "Product Flow",
    ...
    screens: (sim.funnel_drop_off ?? []).map((f, i) => ({
      id: f.screen_id,
      label: f.screen_id.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
      order: i,
    })),
    funnel: (sim.funnel_drop_off ?? []).map((f) => ({
      screen: f.screen_id,
      entered: 0,
      dropped: f.drop_offs,
    })),
    rootCauses: [],
    oneBet: {
      title: sim.flow_assessment?.overall_verdict ?? "",
      rationale: "",
      effort: "",
      impact: "",
      projectedCompletion: "",
  ...
  return (
    <>
      {banner}
      <div className={`${playfair.variable} ${dmSans.variable}`}>
        <FlowAnalysisView data={flowData} simulationData={sim} />
      </div>
    </>
  );
}
