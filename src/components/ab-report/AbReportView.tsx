...
import { Check as CheckIcon, X as XIcon, UserCircle2, Users, Activity, Layers } from "lucide-react";
import type {
  AbReport,
  AnnotatedScreenPair,
...
export function AbReportView({ data }: AbReportViewProps) {
              ...
              <div style={{ background: T.heroBg, borderRadius: 20, padding: "40px 44px", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 20 }}>
                  {[
                    ...
                    <span key={t} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 999, padding: "4px 14px", fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.65)" }}>
                      {t}
                    </span>
                  ))}
              ...
              return (
                <>
                  <SectionHeader
                    ...
                    rightSlot={<CopyButton getText={() => filtered.map((s) => s.markdown).join("\n")} labeled="Copy all as markdown" />}
                  />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {data.monologue_diff.map((m, i) => (
                <motion.div key={m.persona_id} {...stagger(i)} style={{ ...cardStyle, padding: 24 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }} className="monologue-grid">
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 500, color: T.text3, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 10 }}>
                        VARIANT A
                      </div>
                  ...
                  <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${T.border}` }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: T.accent, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 6 }}>
                      WHAT FLIPPED
                    </div>
